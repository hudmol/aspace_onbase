...
class OnbaseClient
  ...
  def add_to_keywords(onbase_id, keywords)
    ...
    # find the not_generated keys and move them to a new array to compare to the fetched keywords from onbase
    non_gen_keys_to_keep = []
    
    keywords.delete_if do |keyword|
      if keyword[0] == 'not_generated'
        non_gen_keys_to_keep.push(keyword[1])
        true
      end
    end
    
    # keep the onBase only data that has the keywords that we know are not generated by the system
    onbase_only_keywords = onbase_keywords['keywords'].select { |k| non_gen_keys_to_keep.include?(k['keywordTypeName'])}

    # push those onto the new set of data to be uploaded
    formatted_keywords = format_keywords(keywords).concat(onbase_only_keywords)
    
    # we now preference the ArchivesSpace keywords
    merged = onbase_keywords.merge('keywords' => formatted_keywords)
